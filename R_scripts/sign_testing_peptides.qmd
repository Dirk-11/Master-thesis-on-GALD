...
prot_file \<- "/Users/Dirkdeboer/Dirk/dtu_thesis/data_analysis_dtu/results/01_from_original/protein_ranking_limma_all.csv" pep_file \<- "/Users/Dirkdeboer/Dirk/dtu_thesis/data_analysis_dtu/results/01_from_evidence/peptide_ranking_limma_all.csv" gald_unique_file \<- "/Users/Dirkdeboer/Dirk/dtu_thesis/out_from_gald_file/gald_unique_proteins_selected.csv"
...
gald_unique_file <- "/Users/Dirkdeboer/Dirk/dtu_thesis/out_from_gald_file/gald_unique_proteins_selected.csv"
...
gald_unique <- readr::read_csv(gald_unique_file, show_col_types = FALSE)
# In your file the gene column is called "gene", not "symbol"
stopifnot("gene" %in% names(gald_unique))
gald_unique_genes <- unique(gald_unique$gene)

cat("GALD-unique gene symbols from file:\n")
print(gald_unique_genes)

# =========================================================
#         ENSURE SYMBOL COLUMN EXISTS (RANKING TABLES)
# =========================================================
...
# small sanity check: how many unique genes present in each table?
cat("\nOverlap with protein ranking table:\n")
print(intersect(gald_unique_genes, unique(prot$symbol)))
cat("N in protein table:", length(intersect(gald_unique_genes, unique(prot$symbol))), "\n")

cat("\nOverlap with peptide ranking table:\n")
print(intersect(gald_unique_genes, unique(pep$symbol)))
cat("N in peptide table:", length(intersect(gald_unique_genes, unique(pep$symbol))), "\n\n")

# =========================================================
#      PREP DF WITH SIGNIFICANCE + GREEN OVERRIDE
# =========================================================
...
make_volcano_df <- function(df) {
  df %>%
    mutate(
      ...
      # original significance classification
      sig_class = case_when(
      ...
      # genes we want to highlight (using gene list from 'gene' column)
      is_unique = symbol %in% gald_unique_genes,

      # final color class (GALD-unique overrides)
      color_class = case_when(
        is_unique ~ "unique",   # bright green
        sig_class == "up"   ~ "up",
        sig_class == "down" ~ "down",
        TRUE                ~ "ns"
      )
...
plot_volcano <- function(df, title) {
  ...
  # labels = top up/down + all unique
  up_lab <- df2 %>%
    filter(sig_class == "up", !is.na(symbol)) %>%
    slice_max(negLogP, n = N_LABEL_UP)

  down_lab <- df2 %>%
    filter(sig_class == "down", !is.na(symbol)) %>%
    slice_max(negLogP, n = N_LABEL_DOWN)

  uniq_lab <- df2 %>%
    filter(is_unique, !is.na(symbol))

  lab_df <- bind_rows(up_lab, down_lab, uniq_lab) %>%
    distinct(symbol, .keep_all = TRUE)

  ggplot(df2, aes(x = logFC, y = negLogP)) +
    geom_point(aes(color = color_class), alpha = 0.7, size = 1.7) +
    scale_color_manual(
      values = c(
        "down"   = "#2f88b0",   # blue
        "ns"     = "grey80",    # grey
        "up"     = "#e31a1c",   # red
        "unique" = "darkgreen"  # green (GALD-unique)
      ),
      breaks = c("down", "ns", "up", "unique"),
      labels = c("Down", "Not sig", "Up", "GALD-unique proteins"),
      name = NULL
    ) +
    ...
    ggrepel::geom_text_repel(
      data = lab_df,
      aes(label = symbol, color = color_class),
      size = 3,
      max.overlaps = 100,
      show.legend = FALSE
    ) +
...
p_prot <- plot_volcano(prot, "Protein-level volcano (GALD vs controls)")
p_pep  <- plot_volcano(pep,  "Peptide-level volcano (GALD vs controls)")

print(p_prot)
print(p_pep)

ggsave("volcano_protein_GALD_unique.png", p_prot, width = 7, height = 6, dpi = 300)
ggsave("volcano_peptide_GALD_unique.png", p_pep, width = 7, height = 6, dpi = 300)
...
# Load limma table
tt <- readr::read_csv(pep_file, show_col_types = FALSE)
